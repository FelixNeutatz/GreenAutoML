...
for i in range(len(my_openml_tasks)):
    ...
    if len(np.unique(y_train)) == 2:
        new_tasks.append(my_openml_tasks[i])

my_openml_tasks = new_tasks

matrix = np.zeros((len(my_openml_tasks), 2))
for i in range(len(my_openml_tasks)):
...
matrix_new = np.zeros((1, 2))
i = 0
X_train, X_test, y_train, y_test, categorical_indicator, attribute_names = get_data('data', randomstate=42, task_id=189866)
matrix_new[i, 0] = len(X_train)
matrix_new[i, 1] = X_train.shape[1]

matrix = scaler.transform(matrix)

matrix_new = scaler.transform(matrix_new)

distances = np.linalg.norm(matrix - matrix_new[0], axis=1)

print(my_openml_tasks[np.argmin(distances)])
