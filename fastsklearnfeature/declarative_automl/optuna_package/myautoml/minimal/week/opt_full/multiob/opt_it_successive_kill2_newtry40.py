...
def run_AutoML(task_id, return_dict, dictionary_felix, trial):
    ...
    if trial.params['use_early_stopping']:
        no_improvement_iterations = trial.params['early_stopping_iterations']
...
def sample_configuration(trial):
    ...
    for random_i in range(repetitions_count):
        ...
        for dataset_i in range(len(validation_datasets)):
            ...
            cur_accuracy, cur_energy = run_force_limit(datasetit, dictionary_felix, trial)
            all_sum += cur_accuracy
            all_sum_energy += -1 * cur_energy
