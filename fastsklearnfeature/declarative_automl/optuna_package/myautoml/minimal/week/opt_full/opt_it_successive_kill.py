...
def sample_configuration(trial):
    ...
    for random_i in range(repetitions_count):
        ...
        current_data = 0
        while True:
            new_datasets = validation_datasets[current_data:min(current_data + parallelism, len(validation_datasets))]
            processes = []
            start_current = time.time()
            for key in new_datasets:
                my_process = Process(target=run_AutoML, name='start' + str(key), args=(key, return_dict, dictionary_felix, trial,))
                my_process.start()
                processes.append(my_process)

            for my_process in processes:
                my_process.join((search_time*4) - int(time.time() - start_current))
            ...
            if min(current_data + parallelism, len(validation_datasets)) == len(validation_datasets):
                break
            current_data += parallelism

        all_sum += np.sum(return_dict.values())
