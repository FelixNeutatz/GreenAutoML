...
#40bin
my_openml_tasks = [211722, 211723, 75147, 75146, 273, 75156, 75163, 146592, 146593, 146594, 75174, 189863, 75176, 189864, 167085, 167086, 189870, 189869, 75184, 75192, 167097, 166970, 75195, 167100, 167101, 167099, 167103, 336, 75219, 166996, 75093, 168791, 75225, 75100, 75108, 75237, 75112, 75126, 146679, 75133]
...
def run_force_limit(task_id, dictionary_felix, trial):
    ...
    if not dict_key in dictionary_felix:
        dummyclf = sklearn.dummy.DummyClassifier()
        dummyclf.fit(X_train, y_train)
        y_hat_test = dummyclf.predict(X_test)
        dummy_score = balanced_accuracy_score(y_test, y_hat_test)

        dictionary_felix[str(dict_key) + '_dummy'] = dummy_score
