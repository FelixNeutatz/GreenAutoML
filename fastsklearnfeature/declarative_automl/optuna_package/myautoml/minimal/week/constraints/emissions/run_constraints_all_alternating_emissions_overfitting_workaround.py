...
def sample_and_evaluate(my_id1):
    if time.time() - starting_time_tt > 60*30*1:#60*60*24*1: # 60*60*24*7
    ...
    if True:
        ...
        if use_overfitting:
            ...
            # pick instances created by overfitting and learn
            ...
            if type(None) == type(model_overfitting):
                ...
                #use overfitting model to find ML task
                best_trial = get_best_trial(model_overfitting,
                                            total_search_time=total_search_time,
                                            my_openml_tasks=my_openml_tasks,
                                            my_openml_tasks_fair=my_openml_tasks_fair,
                                            feature_names=feature_names,
            ...
            #search for best configuration for the given ML task based on surrogate model
            ...
            study_prune = optuna.create_study(direction='maximize')  # TODO
            study_prune.optimize(lambda trial: optimize_accuracy_under_minimal_sample_ensemble(trial=trial,
                                                                                                         metafeature_values_hold=metafeature_values,
                                                                                                         search_time=search_time_frozen,
                                                                                                         model_success=model_uncertainty,
                                                                                                         memory_limit=10,
                                                                                                         privacy_limit=None,
                                                                                                         training_time_limit=None,
                                                                                                         inference_time_limit=None,
            ...
            features_of_sampled_point = study_prune.best_trial.user_attrs['best_features']
            specified_space = study_prune.best_trial.user_attrs['space']
        else:
