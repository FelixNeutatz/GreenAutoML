...
def sample_and_evaluate(my_id1, starting_time_tt, total_search_time, my_scorer, dictionary, my_openml_tasks, my_openml_tasks_fair, feature_names, my_lock):
...
if __name__ == "__main__":
    ...
    with NestablePool(processes=topk, initializer=init_pool_processes, initargs=(my_lock,)) as pool:
        results = pool.map(partial(sample_and_evaluate,
                                   ...
                                   my_openml_tasks_fair=my_openml_tasks_fair,
                                   feature_names=feature_names,
                                   my_lock=my_lock), range(2))#100000
